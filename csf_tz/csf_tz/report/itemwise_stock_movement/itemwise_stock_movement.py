...
def execute(filters=None):
	...
	if sl_entries:
		...
		pvt = pd.pivot_table(
			df, values="actual_qty", index=["posting_date", "Particulars"], columns="item_code", fill_value=0
		)
...
def get_stock_ledger_entries(filters, items):
	...
	return frappe.db.sql(
		"""SELECT	sle.posting_date,
							...
							ORDER BY sle.posting_date asc""".format(
	...
	return frappe.db.sql(
		"""SELECT	STR_TO_DATE(%(from_date)s, '%%Y-%%m-%%d') as posting_date,
							...
							WHERE sle.company = %(company)s
									...
									{sle_conditions}
									{item_conditions_sql}
							GROUP BY `Particulars`, sle.item_code""".format(
			sle_conditions=get_sle_conditions(filters), item_conditions_sql=item_conditions_sql
		),
		filters,
		as_dict=1,
