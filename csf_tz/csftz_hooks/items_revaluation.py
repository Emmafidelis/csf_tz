...
def get_stock_ledger_entries(report_filters):
	...
	query = (
		frappe.qb.from_(sle).select(*(getattr(sle, field) for field in fields)).where(sle.is_cancelled == 0)
	)
	...
	return (
		query.orderby(Timestamp(sle.posting_date, sle.posting_time)).orderby(sle.creation).run(as_dict=True)
	)
